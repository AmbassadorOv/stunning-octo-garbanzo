...
- Optional dependencies (for better GPU monitoring): pynvml, psutil, pypdf
"""
...
class ConceptTree:
    ...
    def add_node(self, node_id: str, data: Dict[str, Any], parent_id: Optional[str] = None):
        if node_id in self.nodes:
            raise ValueError(f"Node {node_id} already exists.")
        self.nodes[node_id] = {"id": node_id, "data": data, "children": []}
        if parent_id:
    ...
    def get_children(self, node_id: str) -> List[str]:
        return self.nodes.get(node_id, {}).get("children", [])
...
# -----------------------
class Agent:
    def __init__(self, id: str, tree: ConceptTree, orchestrator: "Orchestrator", memory: Optional[Dict] = None):
        self.id = id
        self.tree = tree
    ...
    async def handle_message(self, message: str) -> Dict[str, Any]:
        ...
        concept = self.tree.nodes[self.id]['data']['content']
        zodiac_sign = message
        prompt = (
            f"To Beverly, regarding the zodiac sign {zodiac_sign}, "
            f"consider the quantum concept of {concept}: "
            "Explain the connection in a short, poetic, and mystical way."
        )
        # Use orchestrator's gpt wrapper for generation
        gen = await self.orchestrator.gpt.generate(prompt, max_length=128)
    ...
    async def forward_to_children(self, message: str):
        for nid in self.tree.get_children(self.id):
...
# -----------------------
class Orchestrator:
    def __init__(self, tree: ConceptTree, gpt: GPTWrapper, cpu_workers: int = 4, gpu_threshold: float = 70.0):
        self.tree = tree
        self.gpt = gpt
    ...
    def register_all_agents(self):
        for nid in self.tree.get_node_ids():
            a = Agent(nid, self.tree, self)
...
async def gpt_task_automation(orchestrator: Orchestrator, input_text: str) -> Dict[str, Any]:
    ...
    agent_id = random.choice(orchestrator.tree.get_node_ids())
...
def parse_concepts(filepath: str) -> ConceptTree:
    if filepath.endswith(".pdf"):
        try:
            text = extract_text_from_pdf(filepath)
            lines = text.split("\n")
        except ImportError:
            print("Error: pypdf is not installed. Please install it to use PDF files.", file=sys.stderr)
            sys.exit(1)
    else:
    ...
    tree = ConceptTree()
    tree.add_node("root", data={"content": "Quantum Concepts"})
    for line in lines:
        line = line.strip()
        if not line:
            continue
        ...
        if "." in line:
            ...
            if len(parts) > 1 and parts[0].isdigit():
                ...
                tree.add_node(node_id, data={"content": title}, parent_id="root")
    return tree
...
async def run_demo(num_tasks: int, use_local_hf: bool = False, gpu_threshold: float = 70.0, input_file: str = "quantum_concepts.md") -> str:
    """
    Run a single demo execution: build tree, orchestrator, dispatch tasks equal to num_tasks,
    ...
    tree = parse_concepts(input_file)

    orchestrator = Orchestrator(tree, gpt, cpu_workers=4, gpu_threshold=gpu_threshold)
    orchestrator.register_all_agents()

    # Build tasks (one message per node up to num_tasks)
    node_ids = tree.get_node_ids()
    tasks = []
    ...
    out = {"timestamp": timestamp, "metrics": orchestrator.metrics, "results": results}
...
def parse_args():
    ...
    dsp.add_argument("--input-file", type=str, default="quantum_concepts.md", help="Input file for concepts (md or pdf)")

    rtp = sub.add_parser("run-text", help="Run single text through GPT_TASK automation")
    rtp.add_argument("zodiac_sign", type=str, help="Zodiac sign to use in the prompt")
    rtp.add_argument("--use-local-hf", action="store_true")
    rtp.add_argument("--gpu-threshold", type=float, default=70.0)
    rtp.add_argument("--input-file", type=str, default="quantum_concepts.md", help="Input file for concepts (md or pdf)")
...
def main_cli():
    args = parse_args()
    if args.cmd == "demo":
        asyncio.run(run_demo(args.tasks, use_local_hf=args.use_local_hf, gpu_threshold=args.gpu_threshold, input_file=args.input_file))
        ...
        async def run_one():
            ...
            tree = parse_concepts(args.input_file)
            orch = Orchestrator(tree, gpt, cpu_workers=2, gpu_threshold=args.gpu_threshold)
            orch.register_all_agents()
            result = await gpt_task_automation(orch, args.zodiac_sign)
